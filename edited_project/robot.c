...
void robotAutoMotorMove(struct Robot * robot, int front_centre_sensor, int left_sensor, int right_sensor, int right_prev, int left_prev) {
    ...
    printf("Consec right = %d\n", robot->consec_right);
    printf("difference = %d\n", (robot->angle - robot->last90)%90);
    //printf("angle current = %d, last90 = %d\n", robot->angle, robot->last90);
    ...
    if (front_centre_sensor == 0 && right_sensor <= 2 && left_sensor <= 2) {
        ...
        //             robot->currentSpeed = MAX_ROBOT_SPEED;
        robot->direction = UP;
        if (right_prev >= 1 && right_sensor == 0 && (robot->consec_right <= 3)) //robot->angle%90 != 0 ||
        {
            printf("no right wall if entered\n");
            robot->direction = RIGHT;
            //robot->consec_right += 1;
            //robot->consec_left = 0;
            if ((robot->angle - robot->last90)%90 == 0 && (robot->angle - robot->last90) != 0) {
                printf("right consec\n");
                robot->consec_right += 1;
                robot->consec_left = 0;
                if (robot->angle%90 == 0) {
                    robot->last90 = robot->angle;
                }
            }
            
            //robot->watch_loop = 1;
        }
        else if (left_prev >= 1 && left_sensor == 0 && (robot->consec_left <= 3))
        { //not a right angle or must be consec less than or equal to 3.
            robot->direction = LEFT;
            //robot->consec_left += 1;
            //robot->consec_right = 0;
            //if (robot->angle - robot->last90 == -90 || robot->angle - robot->last90 == 270) {
            if ((robot->angle - robot->last90)%90 == 0 && (robot->angle - robot->last90) != 0) {
                printf("left consec\n");
                robot->consec_left += 1;
                robot->consec_right = 0;
    ...
    else 
    {
        ...
        if((left_sensor == 0 || right_sensor >=3) && (robot->angle%90 != 0 || robot->consec_left <= 3))
        {
            // robot->currentSpeed = 
            robot->direction = LEFT;
            if ((robot->angle - robot->last90)%90 == 0){ //&& (robot->angle - robot->last90) != 0) {
        ...
        else
        {
            ...
            if((right_sensor == 0 || left_sensor >= 3)  && (robot->angle%90 != 0 || robot->consec_right <= 3))
            {
                robot->direction = RIGHT;
                if ((robot->angle - robot->last90)%90 == 0){ //&& (robot->angle - robot->last90) != 0) {
                    printf("right consec\n");
