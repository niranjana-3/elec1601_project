...
void robotAutoMotorMove(struct Robot * robot, int front_centre_sensor, int left_sensor, int right_sensor, int prev_centre, int prev_left, int prev_right) {
...
//            robot->currentSpeed++;
    ...
    else if(robot->currentSpeed > 0 && right_sensor == 0 && prev_right > 0) {
        robot->direction = RIGHT;
    }
    else if(robot->currentSpeed > 0 && (left_sensor == 0 && prev_left > 0)) {
        robot->direction = LEFT;
    }
    else if(robot->currentSpeed > 0 && front_centre_sensor >= 0 ) {
        //if left side is empty
        if(left_sensor == 0 || right_sensor >=2)
        {
            robot->direction = LEFT;
        }
        else
        {
            if(right_sensor == 0 || left_sensor >= 2)
            {
                robot->direction = RIGHT;
            }
            else
